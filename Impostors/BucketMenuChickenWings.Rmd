...
df_volcanoplotBRCA_TUMvsNORM_pathways$different[df_volcanoplotBRCA_TUMvsNORM_pathways$foldchange_tum_norm > 0  &     df_volcanoplotBRCA_TUMvsNORM_pathways$p_BRCA_TUMvsNORM_pathways < 1.009123e-30]= "UP"

df_volcanoplotBRCA_TUMvsNORM_pathways$different[df_volcanoplotBRCA_TUMvsNORM_pathways$foldchange_tum_norm < 0 &      df_volcanoplotBRCA_TUMvsNORM_pathways$p_BRCA_TUMvsNORM_pathways < 1.009123e-30]= "DOWN"

#we plot 
ggplot (data = df_volcanoplotBRCA_TUMvsNORM_pathways, aes (x=foldchange_tum_norm, y=-log10(p_BRCA_TUMvsNORM_pathways), col = different)) + ggtitle("volcanoplot BRCA tumour vs norm pathways") + geom_point() + theme_minimal() + scale_color_manual(values = c ("pink","black", "lightblue"))+ theme (text=element_text(size = 8) )
...
#we plot 
ggplot (data = df_volcanoplotBRCAvsKIRCpathways, aes (x=foldchange11, y=-log10(p_BRCAvsKIRCpathways), col = different)) + ggtitle("volcanoplot BRCA tumour vs KIRC tumour pathways") + geom_point() + theme_minimal() + scale_color_manual(values = c ("pink","black", "lightblue"))+ theme (text=element_text(size = 8) )

View(df_volcanoplotBRCAvsKIRCpathways)
...
#we plot 
ggplot (data = df_volcanoplotBRCAvsPRADpathways, aes (x=foldchange33, y=-log10(p_BRCAvsPRADpathways), col = different)) + ggtitle("volcanoplot BRCA tumour vs PRAD tumour pathways") + geom_point() + theme_minimal() + scale_color_manual(values = c ("pink","black", "lightblue"))+ theme (text=element_text(size = 8) )
...
pee_BRCAvsTHCApathways = c()
 for (i in 1:nrow(BRCApathways)){
  pee_BRCAvsTHCApathways[i] = wilcox.test(unlist(BRCApathways[i,]),unlist(THCApathways[i,]),paired = FALSE )$p.value
 }
