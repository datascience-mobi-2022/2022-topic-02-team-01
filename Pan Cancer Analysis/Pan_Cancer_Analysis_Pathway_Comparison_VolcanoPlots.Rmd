...
# here we check if the brca patients in the SkinnyBoy and the SkinnyBRCA's the same people

What we learned: Sample IDS from skinnySADtt are in SB, but the ones from skinnyHAPPYBRCA are not

```{r, echo=FALSE}

SkinnyBRCA_Samplenames = colnames(pw_activity_SkinnyBRCA)

SkinnyBoy_Samplenames = colnames(SkinnyBoy)

SkinnySadBRCA_Samplenames = colnames(SkinnySadBRCA)

match(SkinnyBRCA_Samplenames, SkinnyBoy_Samplenames)

match(SkinnySadBRCA_Samplenames, SkinnyBoy_Samplenames)

```

_______________________________________________________________________________________________________________________________________________________

Here we make comparisons of pathway between BRCA tum and norm as well as BRCA and SB
...
pea_BRCA_TUMvsNORM_pathways = c()
 for (i in 1:nrow(tummyBRCApathways)){
  pea_BRCA_TUMvsNORM_pathways[i] = wilcox.test(unlist(tummyBRCApathways[i,]),unlist(normieBRCApathways[i,]),paired = FALSE )$p.value
 }
p_BRCA_TUMvsNORM_pathways = pea_BRCA_TUMvsNORM_pathways*length(tummyBRCApathways[,1])
...
pea_BRCAvsSBpathways = c()
 for (i in 1:nrow(BRCApathways1)){
  pea_BRCAvsSBpathways[i] = wilcox.test(unlist(BRCApathways1[i,]),unlist(MUTUALpathways[i,]),paired = FALSE )$p.value
 }
p_BRCAvsSBpathways = pea_BRCAvsSBpathways*length(BRCApathways1[,1])
...
In the second part of the RMD file we run the volcano plots for tumour comprisons for pathways (BRCA vs KIRC,PRAD,LUAD)
...
pea_BRCAvsKIRCpathways = c()
 for (i in 1:nrow(BRCApathways)){
  pea_BRCAvsKIRCpathways[i] = wilcox.test(unlist(BRCApathways[i,]),unlist(KIRCpathways[i,]),paired = FALSE )$p.value
 }
p_BRCAvsKIRCpathways = pea_BRCAvsKIRCpathways*length(BRCApathways[,1])
...
pea_BRCAvsLUADpathways = c()
 for (i in 1:nrow(BRCApathways)){
  pea_BRCAvsLUADpathways[i] = wilcox.test(unlist(BRCApathways[i,]),unlist(LUADpathways[i,]),paired = FALSE )$p.value
 }
p_BRCAvsLUADpathways = pea_BRCAvsLUADpathways*length(BRCApathways[,1])
...
pea_BRCAvsPRADpathways = c()
 for (i in 1:nrow(BRCApathways)){
  pea_BRCAvsPRADpathways[i] = wilcox.test(unlist(BRCApathways[i,]),unlist(PRADpathways[i,]),paired = FALSE )$p.value
 }
p_BRCAvsPRADpathways = pea_BRCAvsPRADpathways*length(BRCApathways[,1])
...
#try1
#pea_BRCAvsTHCApathways = c()
 #for (i in 1:nrow(as.numeric(nrow(BRCApathways)))){
  #pea_BRCAvsTHCApathways[i] = wilcox.test(as.numeric(unlist(THCApathways[i,])),as.numeric(unlist(BRCApathways[i,])),paired = FALSE )$p.value
 #}
#p_BRCAvsTHCApathways = pea_BRCAvsTHCApathways*length(BRCApathways[,1])


#try2
pea_BRCAvsTHCApathways = c()
 for (i in 1:nrow(BRCApathways)){
  pea_BRCAvsTHCApathways[i] = wilcox.test(as.numeric(THCApathways[i,]),as.numeric(BRCApathways[i,]),paired = TRUE )$p.value
 }
p_BRCAvsTHCApathways = pea_BRCAvsTHCApathways*length(BRCApathways[,1])
...
pea_BRCAvsTHCApathways = c()
 for (i in 1:nrow(BRCApathways)){
  pea_BRCAvsTHCApathways[i] = wilcox.test(unlist(THCApathways[i,]),unlist(BRCApathways[i,]),paired = FALSE )$p.value
 }
p_BRCAvsTHCApathways = pea_BRCAvsTHCApathways*length(BRCApathways[,1])
